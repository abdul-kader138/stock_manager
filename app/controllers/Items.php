...
class Items extends MY_Controller {
    ...
    public function get_items($alerts = NULL) {
        ...
        $this->datatables
            ->select("items.id as id, image, ".$this->db->dbprefix('items').".code, ".$this->db->dbprefix('items').".name, ".$this->db->dbprefix('categories').".name as cname, store_name, quantity, unit, alert_quantity")
            ->from('items')
            ->join('categories', 'categories.id=items.category_id', 'left')
            ->join('stores', 'stores.id=items.store_id', 'left')
            ->group_by('items.id');
            if ($alerts) {
                $this->datatables->where('quantity < alert_quantity', NULL, FALSE);
            }
            if(!$this->Admin){
				$this->datatables->where('store_id = '. $this->store_id, NULL, FALSE);
			}
