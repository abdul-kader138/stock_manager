...
class Welcome_model extends CI_Model {
    ...
    public function getCheckinQuantity($user_id = NULL) {
        if(!$this->Admin) {
            $user_id = $this->session->userdata('user_id');
        }
        $this->db->select('date_format(date, \'%Y-%m\') as month, sum(quantity) as check_ins')
        ->where($this->db->dbprefix('check_in').".date >= date_sub( now( ) , INTERVAL 12 MONTH ) ", NULL, FALSE)
        ->group_by('date_format(date, \'%Y-%m\')')
		
		->join($this->db->dbprefix('check_in_items'), $this->db->dbprefix('check_in').'.id='. $this->db->dbprefix('check_in_items').'.check_in_id')
        ->order_by('month desc');
        if($user_id) {
             $this->db->where('created_by', $user_id);
        }
    ...
    public function getCheckoutQuantity($user_id = NULL) {
        if(!$this->Admin) {
            $user_id = $this->session->userdata('user_id');
        }
        $this->db->select('date_format(date, \'%Y-%m\') as month, sum(quantity) as check_outs')
        ->where($this->db->dbprefix('check_out').".date >= date_sub( now( ) , INTERVAL 12 MONTH ) ", NULL, FALSE)
        ->group_by('date_format(date, \'%Y-%m\')')
		->join($this->db->dbprefix('check_out_items'), $this->db->dbprefix('check_out').'.id='. $this->db->dbprefix('check_out_items').'.check_out_id')
        ->order_by('month desc');
        if($user_id) {
             $this->db->where('created_by', $user_id);
			 $this->db->where('store_id', $this->store_id);
